...
#OMP_NUM_THREADS=16 GOMP_CPU_AFFINITY="$((th1+0))-$((th1+7)),$((th1+128))-$((th1+OMP_NUM_THREADS+120-1))" numactl -i all /home/mdelapierre/hpc-dummies/misc/xthi_aocc >0.affinity.test.out
...
for i in `seq 1 1 8`;
do
	for line_id in `seq 1 1 $num_rows`; # 0-7.
	do 
...
for i in `seq 9 1 16`;
do
        for line_id in `seq 1 1 $num_rows`; # 0-7, 128-135.
        do
                export OMP_NUM_THREADS=$i
