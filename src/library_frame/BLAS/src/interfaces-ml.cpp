...
long long run_compare_gemm_impl(int m, int k, int n, bool useML, size_t num_of_duplicate, test_class* lib) {
  ...
  for (i = 0; i < num_threads; i++) {
    ...
    int nt = setup_threads_with_predictor(predictor, useML, predictor.model->predict_num_cores(m, k, n, routine_name));
...
long long run_compare_symm_impl(int m, int n, bool useML, size_t num_of_duplicate, test_class* lib) {
  ...
  Predictor predictor("xgb", routine_name);
  int nt = setup_threads_with_predictor(predictor, useML, predictor.model->predict_num_cores(m, n, routine_name));
...
long long run_compare_syrk_impl(int n, int k, bool useML, size_t num_of_duplicate, test_class* lib) {
  ...
  Predictor predictor("xgb", routine_name);
  int nt = setup_threads_with_predictor(predictor, useML, predictor.model->predict_num_cores(n, k, routine_name));
...
// Template implementation for GEMV
template<typename T>
long long run_compare_gemv_impl(int m, int n, bool useML, size_t num_of_duplicate, test_class* lib) {
  std::chrono::high_resolution_clock::time_point t1, t2;
  int64_t time;
  std::chrono::high_resolution_clock::time_point t_1, t_2, t_3, t_4, t_5;
  int64_t time1, time2, time3, time4;
  ...
  //assign space using std::vector for automatic memory management
  std::vector<T> A(static_cast<size_t>(num_of_duplicate) * m * n); //strided array of n matrix
  std::vector<T> x(static_cast<size_t>(num_of_duplicate) * n);
  std::vector<T> y(static_cast<size_t>(num_of_duplicate) * m);

  std::cout << "Trying to alloc GB:" << std::endl;
  std::cout << ((unsigned long long)m*n)*sizeof(T) * num_of_duplicate / 1e9 << std::endl;
  if (A.empty() || x.empty() || y.empty()) {
      std::cout << "Trying to alloc:" << std::endl;
    std::cout << static_cast <size_t>(num_of_duplicate)*m*n*sizeof(T) << std::endl;
      std::cout << "Allocation error." << std::endl;
    }
  ...
  size_t stride = static_cast<size_t>(num_of_duplicate) * m * n / num_threads;
    ...
    for (i = 0; i < num_threads; i++) {
        struct t_arg *arg_ptr = (struct t_arg *)malloc(sizeof(struct t_arg));
        arg_ptr->M = A.data(); // matrix start address
        arg_ptr->start = i * stride; // thread start address
        arg_ptr->type = get_type_char<T>(); // double or float
        if (i == num_threads - 1) arg_ptr->end = static_cast<size_t>(num_of_duplicate) * m * n;
  ...
  for (i = 0; i < num_threads; i++) {
    ...
    // fill x
  stride = static_cast<size_t>(num_of_duplicate) * n / num_threads;
    for (i = 0; i < num_threads; i++) {
        struct t_arg *arg_ptr = (struct t_arg *)malloc(sizeof(struct t_arg));
    arg_ptr->M = x.data();
        arg_ptr->start = i * stride;
    arg_ptr->type = get_type_char<T>();
    if (i == num_threads - 1) arg_ptr->end = static_cast<size_t>(num_of_duplicate) * n;
        else arg_ptr->end = (i+1) * stride;
        arg_ptr->seed = i;
        rc = pthread_create(&threads[i], NULL, fill_with_rand, (void *)arg_ptr);
  ...
  for (i = 0; i < num_threads; i++) {
    ...
    // Use predictor to set threads
  std::string routine_name = std::string(get_blas_prefix<T>()) + "gemv";
  Predictor predictor("xgb", routine_name);
  int nt = setup_threads_with_predictor(predictor, useML, predictor.model->predict_num_cores(m, n, routine_name));
  ...
  for (int i = 0; i < num_of_duplicate; i++) {
    ...
    if constexpr (std::is_same_v<T, double>) {
      lib->dgemv(m, n, 1.0, &A[static_cast<size_t>(i)*m*n], &x[static_cast<size_t>(i)*n], 0.0, &y[static_cast<size_t>(i)*m]);
    } else {
      lib->sgemv(m, n, 1.0f, &A[static_cast<size_t>(i)*m*n], &x[static_cast<size_t>(i)*n], 0.0f, &y[static_cast<size_t>(i)*m]);
    }
