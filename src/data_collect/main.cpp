...
void write_csv_results(rapidcsv::Document& csv_file, const std::vector<long long>& results, 
  ...
  // Validate that results vector has sufficient elements
  if (static_cast<int>(results.size()) < num_of_duplicate) {
    throw blas_exceptions::InvalidArgumentException("results", 
      "vector size (" + std::to_string(results.size()) + ") is less than num_of_duplicate (" + std::to_string(num_of_duplicate) + ")");
  }
...
int handle_gemm(test_class* lib, rapidcsv::Document& csv_file, const std::string& domain_file_name, 
  ...
  try {
    ...
    throw; // Re-throw BLAS exceptions
  } catch (const std::exception& e) {
    throw blas_exceptions::CSVException("write", "error writing results: " + std::string(e.what()));
  }
